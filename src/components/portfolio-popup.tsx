...
import { ExternalLink, User, ChevronDown, Info } from "lucide-react";
  ...
  if (isDesktopMinimized) {
    return (
        <div
            ...
            role="button"
            tabIndex={0}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleDesktopToggleMinimize(); }}
            aria-label={t('portfolioPopup.expand.ariaLabel')}
        >
          <Avatar className="h-12 w-12 border-2 border-primary/70 shadow-lg group-hover:scale-110 transition-transform duration-200">
            <AvatarImage src="https://placehold.co/48x48.png?text=LL" alt="Lucas Lima" data-ai-hint="avatar person" />
        ...
        <CardHeader className="flex flex-row items-center justify-between p-3">
          <div className="flex items-center space-x-2">
            <Avatar className="h-10 w-10 border-2 border-primary/50">
              <AvatarImage src="https://placehold.co/40x40.png?text=LL" alt="Lucas Lima" data-ai-hint="avatar person" />
          ...
          <div className="relative flex items-center">
            ...
            <Button
                ...
                onClick={handleDesktopToggleMinimize}
                aria-label={t('portfolioPopup.minimize.ariaLabel')}
            >
              <ChevronDown size={18} />
            </Button>
          </div>
